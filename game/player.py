...
from constants.direction import UP, DOWN, LEFT, RIGHT
...
class Player(GameObject):
    ...
    def update(self):
        ...
        if (keys[pygame.K_LEFT] and not self.rect[0] <= 0):
            ...
            self.direction = UP
            self.speed = PLAYER_SHIP_SPEED
        elif (keys[pygame.K_RIGHT] and not SCREEN_WIDTH - SPRITE_FRAME_WIDTH <= self.rect[0] ):
            self.direction = RIGHT
            self.speed = PLAYER_SHIP_SPEED
        elif (keys[pygame.K_DOWN] and not SCREEN_HEIGHT - SPRITE_FRAME_HEIGHT <= self.rect[1]):
            self.direction = DOWN
            self.speed = PLAYER_SHIP_SPEED
        else:
            self.speed = 0
